...
src-tauri/
  ...
  src/database.rs              SQLite repository / migration
  src/manager.rs               服务生命周期 Facade
  src/proxy/http.rs            HTTP reverse/forward/CONNECT
  src/proxy/rewrite.rs         Header 与 body rewrite
  src/proxy/tcp.rs             TCP forward
  src/proxy/udp.rs             UDP forward
  src/proxy/ssh.rs             SSH local/remote forward 与 SOCKS5 动态代理
