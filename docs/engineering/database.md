...
- 迁移文件：`src-tauri/migrations/0001_initial.sql`、`0002_indexes.sql`、`0003_ssh_jump_profiles.sql`、`0004_tool_services.sql`、`0005_tool_service_static_mode.sql`。
...
- 默认设置：启动时补齐 `app.initialized`、`logs.retention_days`、`logs.max_rows`、`services.auto_start_enabled`、`ui.theme_mode`，不会覆盖用户已保存的值。
...
| `tool_services` | 本地服务页 HTTP 工具服务的名称、监听地址、静态目录和静态目录访问模式。 |
...
- 工具服务必须具备名称、监听主机和端口，并至少配置静态目录或一个接口；静态目录模式只能是 `directory` 或 `site`；文件响应必须通过路径字段引用本地文件。
...
- migration 创建并补齐默认设置，包含 `ui.theme_mode="system"`。
- 应用数据目录默认解析和环境变量覆盖。
- 服务 CRUD roundtrip 保留 header/body rules。
- 工具服务配置 roundtrip 保留静态目录模式、接口路由、编辑更新和暂停状态摘要。
...
真实 SSH server、Windows 系统代理注册表和安装包 smoke 的验收范围见 [testing.md](testing.md)。本文档只维护 SQLite schema、迁移和数据访问边界。
