...
- 数据库文件：`proxy-tool.db`。
- 位置：Tauri `app_data_dir()`。
- 测试/验收可通过 `NET_POWER_APP_DATA_DIR` 覆盖 app data 目录；未设置时使用 Tauri 系统目录。
- 迁移文件：`src-tauri/migrations/0001_initial.sql`、`0002_indexes.sql`、`0003_ssh_jump_profiles.sql`。
- Rust 数据访问：`src-tauri/src/database.rs`，基于 `rusqlite` 和 `Mutex<Connection>`。
